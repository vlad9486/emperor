...
#define CHECK_READ_PERMISSION(m, a) \
    ...
    if (*perror != E__NONE) {\
        *perror |= E_I_ACCESSTABLE;\
        return;\
    }\
    if ((*permission & P_READ) == 0) {\
        *perror = E_I_READATTEMPT;\
        return;\
    }
